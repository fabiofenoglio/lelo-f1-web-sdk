...
const leloF1SdkWebClient = {
    ...
    connectionAttemptProgressive: 0,
    ...
    requestDevice: async function() {
        ...
        try {
            result = await navigator.bluetooth.requestDevice({
                filters: leloF1SdkConstants.COMPATIBLE_DEVICE_NAMES.map((deviceName) => {
                    return {
                        name: deviceName
                    };
                }),
    ...
    connect: async function(device) {
        this.disposing = false;
        this.connecting++;
        this.currentConnectionAttempt = ++this.connectionAttemptProgressive;
        ...
        this.protocol = device.name.toUpperCase().includes("V2") ? 2 : 1;
        this.logger.debug('discovering protocol from device name: ' + device.name + ' => ' + this.protocol);
        this.discovering++;

        try {
            await this._discoverServices(device, server);
        } catch (err) {
    ...
    _discoverServices: function(device, server) {
        ...
        return server.getPrimaryServices().then(services => {
            ...
            for (const entry of leloF1SdkConstants.USED_SERVICES) {
                context.logger.debug('searching for needed device service', entry.name);

                promises.push(
                    server.getPrimaryService(entry.uuid).then((service) => {
                        if (!service) {
    ...
    _discoverServiceCharacteristics: function(entry, service) {
        ...
        return service.getCharacteristics().then((characteristics) => {
            ...
            for (const charEntry of entry.characteristics) {
                ...
                charPromises.push(
                    service.getCharacteristic(charEntry.uuid).then((characteristic) => {
    ...
    _waitForAuthorizationF1sV1: function() {
        const context = this;
        const attemptNum = context.currentConnectionAttempt;
        return new Promise(function(resolve, reject) {
            const connectStatus = {
                pending: 0,
            };
            ...
            const handler = setInterval(function() {
                ...
                if (context.currentConnectionAttempt !== attemptNum || context.disposing) {
    ...
    getBatteryLevel: function() {
        return this._read(leloF1SdkDeviceDefinitions.BATTERY_SERVICE, leloF1SdkDeviceDefinitions.BATTERY_LEVEL)
    ...
    _notImplemented: function() {
        return Promise.reject('NOT IMPLEMENTED - please open an issue at https://github.com/fabiofenoglio/lelo-f1-web-sdk/issues if you need this feature.');
    }
};
