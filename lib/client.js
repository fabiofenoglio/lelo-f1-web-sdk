...
const leloF1SdkWebClient = {
    ...
    _withLock: function(promiseBuilder, lockProperty = 'bleReadWrite') {
        const context = this;
        ...
        return new Promise(function(resolve, reject) {
            ...
            function attemptDispatching() {
                ...
                try {
                    context.logger.debug('acquired BLE access lock ' + lockProperty);
                    const promise = promiseBuilder();

                    promise.then(function(cbResult) {
                        context[lockProperty] --;
                        context.logger.debug('released BLE access lock ' + lockProperty);
                        if (handler) {
                    ...
                    context.logger.error('error in routine inside BLE access lock ' + lockProperty, err);
                    context[lockProperty] --;
                    context.logger.debug('released BLE access lock ' + lockProperty);
                    if (handler) {
                        clearInterval(handler);
            ...
            if (!dispatched) {
                context.logger.debug('BLE access lock ' + lockProperty + ' is busy. Will wait for lock.');
    ...
    shutdown: function() {
        ...
        return this._write(leloF1SdkDeviceDefinitions.LELO_SERVICE, leloF1SdkDeviceDefinitions.MOTOR_CONTROL, Uint8Array.of(1, 15, 10)).then(function() {
            context.logger.debug('shut down signal sent, disconnecting');
